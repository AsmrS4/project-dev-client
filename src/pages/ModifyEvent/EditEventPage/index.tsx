...
import type { EventCreate, IEvent } from 'src/models/Event/Event';
...
const EditEventPage = () => {
    ...
    const [eventDetails, setDetails] = useState<IEvent>(init);
    ...
    const editEvent = async (data: EventCreate) => {
        try {
            const response = await axios({
                url: `${'http://localhost:8090/api'}/event/${id}`,
                method: 'PUT',
    ...
    useEffect(() => {
        ...
        setImageUrls(eventDetails.images);
        setDateTime(moment(eventDetails.dateTime));
        console.log('loop');
    }, [eventDetails]);
    ...
    return (
        <section className={styles.eventPage}>
            ...
            <ActionButton title={'Изменить'} type={'submit'} onClick={handleSubmit} />
            <ActionButton
                title={'Отмена'}
                type={'submit'}
                onClick={() => {
